...
Astro.eventManager.on('initClass', function() {
  ...
  Class.isAuthorized = function(userId,rt,SO) {//SO = SecurityObject (either ACL or CLP)
  ...
  Class.autorizeRequest = function(userId,rt,acl){
      //evaluate write request
      console.log(Class.getACLRT(rt));
        return Class.isAuthorized(userId,rt, schema.security.CLP)? (acl? Class.isAuthorized(userId,Class.getACLRT(rt), acl):true):false;
    };
  Class.buildQuery = function(inputQueryArray,userId){
    var aclOrQuery = [{
        'ACL.controls':{
          $elemMatch:{
            'type.name':'public',
      ...
      var andQuery = [{$or:aclOrQuery}].concat(inputQueryArray||[]);
      var query = {$and:andQuery};
    ...
    Class.generateSecureMethodsAndPublications = function(){
      ...
      if (types['insert']){
         methods['insert' + schema.className] = function(values){
          ...
          if (!Class.autorizeRequest(this.userId,'insert',null)) throw new Meteor.Error('404', 'Not authorized to update the object');
      ...
      if (types['update']){
        methods['update' + schema.className] = function(ciId,values) {
          ...
          if (!Class.autorizeRequest(this.userId,'update',acl)) throw new Meteor.Error('404', 'Not authorized to update the object');
          //Set the values that the update requested
          ci.set(values);
          //See if the ci object passes the validators
          if (!ci.validate()) throw new Meteor.Error('403', 'Not a valid ci');
      ...
      if (types['find']) {
        ...
        Meteor.publish(schema.className,function(){
            ...
            if (!Class.autorizeRequest(this.userId,'find',null)) throw new Meteor.Error('404', 'Not authorized to find objects in this class');
